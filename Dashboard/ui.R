...
server <- function(input, output) {
  ...
  output$Prediction <- renderUI({
    if(input$Method=="ESR" && input$SBC !=0 )
    {
      ...
      pyImport("RandomForestRegressor","sklearn.ensemble")
      MPCC <- pyExec(RandomForestRegressor(n_estimators = 5, random_state = 0))
      print(paste("MPCC"))
      MRCC <- RandomForestRegressor(RCC ~ ., data=traindata[c(1,3,4,5,7,8,10)], ntree=5)
      Mfw <-RandomForestRegressor(Formwork ~ ., data=traindata[c(1,3,4,5,7,8,11)], ntree=5)
      MSteel <-RandomForestRegressor(Rft ~ ., data=traindata[c(1,3,4,5,7,8,12)], ntree=5)
      Mrate <-RandomForestRegressor(RatePerLit ~ ., data=traindata[c(1,3,4,5,7,8,13)], ntree=5)
     ...
     #             "\n Prdicted Cost = ",predCost))
      ...
      HTML(paste("<table border= 1 class='table w-50 mx-auto'  ><tr align ='center'><td>RCC </td>","<td>",predRCC,"</td></tr>",
                 ...
                 "<tr align ='center'><td>Rate per litre </td>" ,"<td>",predrate,"</td></tr>",
    ...
    else
    {
      ...
      MPCC <- pyExec(RandomForestRegressor(n_estimators = 5, random_state = 0))
      print(paste("MPCC"))
      MPCC_NoSBC <- RandomForestRegressor(PCC ~ ., data=traindata_Nosbc[c(1,3,4,6,7,8)], ntree=5)
      MRCC_NoSBC <- RandomForestRegressor(RCC ~ ., data=traindata_Nosbc[c(1,3,4,6,7,9)], ntree=5)
      Mfw_NoSBC <-RandomForestRegressor(Formwork ~ ., data=traindata_Nosbc[c(1,3,4,6,7,10)], ntree=5)
      MSteel_NoSBC <-RandomForestRegressor(Rft ~ ., data=traindata_Nosbc[c(1,3,4,6,7,11)], ntree=5)
      Mrate_NoSBC <-RandomForestRegressor(RatePerLit ~ ., data=traindata_Nosbc[c(1,3,4,6,7,12)], ntree=5)
      ...
      HTML(paste("<table border= 1 class='table w-50 mx-auto'  ><tr align ='center'><td>RCC </td>","<td>",predRCC_NoSBC,"</td></tr>",
                 ...
                 "<tr align ='center'><td>Rate per liter </td>" ,"<td>",predrate_NoSBC,"</td></tr>",
  ...
  data1<- reactive({
    if(input$Method=="ESR" && input$SBC !=0 )
    {
      ...
      print(paste(ESR))
      testdata<-ESR[-trainIndex,]
      MPCC <- RandomForestRegressor(PCC ~ ., data=traindata[c(1,3,4,5,7,8,9)], ntree=5)
      MRCC <- RandomForestRegressor(RCC ~ ., data=traindata[c(1,3,4,5,7,8,10)], ntree=5)
      Mfw <-RandomForestRegressor(Formwork ~ ., data=traindata[c(1,3,4,5,7,8,11)], ntree=5)
      MSteel <-RandomForestRegressor(Rft ~ ., data=traindata[c(1,3,4,5,7,8,12)], ntree=5)
      Mrate <-RandomForestRegressor(RatePerLit ~ ., data=traindata[c(1,3,4,5,7,8,13)], ntree=10)
      pred_grid <- data.frame( State=input$State,SesimicZone=input$Zone, WindSpeed=as.numeric(input$Windspeed),   SBC=as.numeric(input$SBC), Capacity = as.numeric(input$Capacity)  , StagingHeight=as.numeric(input$Height) )
      levels(pred_grid$State) <- levels(testdata$State)
      levels(pred_grid$SesimicZone) <- levels(testdata$SesimicZone)
      predPCC<-round(predict(MPCC, pred_grid),2)
    ...
    else
    {
      ...
      Mfw_NoSBC <-RandomForestRegressor(Formwork ~ ., data=traindata_Nosbc[c(1,3,4,6,7,10)], ntree=5)
      MSteel_NoSBC <-RandomForestRegressor(Rft ~ ., data=traindata_Nosbc[c(1,3,4,6,7,11)], ntree=5)
      Mrate_NoSBC <-RandomForestRegressor(RatePerLit ~ ., data=traindata_Nosbc[c(1,3,4,6,7,12)], ntree=10)
      pred_grid <- data.frame( State=input$State,SesimicZone=input$Zone, WindSpeed=as.numeric(input$Windspeed),  Capacity = as.numeric(input$Capacity)  , StagingHeight=as.numeric(input$Height) )
      levels(pred_grid$State) <- levels(traindata_Nosbc$State)
      levels(pred_grid$SesimicZone) <- levels(traindata_Nosbc$SesimicZone)
